...
type CIM_TPM struct { 
	*CIM_LogicalDevice
	...
	func NewCIM_TPMEx1(instance *cim.WmiInstance) (newInstance *CIM_TPM, err error) {tmp, err := NewCIM_LogicalDeviceEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *CIM_TPM, err error) {tmp, err := NewCIM_LogicalDeviceEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
/*Custon IN*/  Timeout uint32) (result uint32, err error) {retVal, err := instance.InvokeMethodAsync("RequestTPMStateChange", Action, PercentComplete, Timeout , RequestedTPMState, AuthorizationToken, TimeoutPeriod)
	if err != nil { return }
...
func (instance *CIM_TPM) ChangeOwnerAuth( /* IN */ OldOwnerAuth string,
 /* IN */ NewOwnerAuth string) (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("ChangeOwnerAuth" , OldOwnerAuth, NewOwnerAuth);
	if err != nil { return }
