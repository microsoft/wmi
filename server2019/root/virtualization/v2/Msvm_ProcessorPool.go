...
type Msvm_ProcessorPool struct { 
	*CIM_ResourcePool
	...
	func NewMsvm_ProcessorPoolEx1(instance *cim.WmiInstance) (newInstance *Msvm_ProcessorPool, err error) {tmp, err := NewCIM_ResourcePoolEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *Msvm_ProcessorPool, err error) {tmp, err := NewCIM_ResourcePoolEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *Msvm_ProcessorPool) CalculatePossibleReserve( /* IN */ ProcessorCount uint16) (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("CalculatePossibleReserve" , ProcessorCount);
	if err != nil { return }
