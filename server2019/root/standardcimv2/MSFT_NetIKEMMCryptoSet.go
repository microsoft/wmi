...
type MSFT_NetIKEMMCryptoSet struct { 
	*MSFT_NetIKECryptoSet
	...
	func NewMSFT_NetIKEMMCryptoSetEx1(instance *cim.WmiInstance) (newInstance *MSFT_NetIKEMMCryptoSet, err error) {tmp, err := NewMSFT_NetIKECryptoSetEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_NetIKEMMCryptoSet, err error) {tmp, err := NewMSFT_NetIKECryptoSetEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_NetIKEMMCryptoSet) Rename( /* IN */ NewName string) (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("Rename" , NewName);
	if err != nil { return }
