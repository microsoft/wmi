...
type MSFT_DNSClientCache struct { 
	*CIM_ManagedElement
	...
	func NewMSFT_DNSClientCacheEx1(instance *cim.WmiInstance) (newInstance *MSFT_DNSClientCache, err error) {tmp, err := NewCIM_ManagedElementEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_DNSClientCache, err error) {tmp, err := NewCIM_ManagedElementEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_DNSClientCache) Clear() (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("Clear" );
	if err != nil { return }
