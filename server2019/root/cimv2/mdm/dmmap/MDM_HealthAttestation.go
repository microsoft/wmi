...
type MDM_HealthAttestation struct { 
	*cim.WmiInstance
	...
	func NewMDM_HealthAttestationEx1(instance *cim.WmiInstance) (newInstance *MDM_HealthAttestation, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MDM_HealthAttestation, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MDM_HealthAttestation) VerifyHealthMethod() (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("VerifyHealthMethod" );
	if err != nil { return }
