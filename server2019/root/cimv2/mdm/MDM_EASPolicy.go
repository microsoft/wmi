...
type MDM_EASPolicy struct { 
	*cim.WmiInstance
	...
	func NewMDM_EASPolicyEx1(instance *cim.WmiInstance) (newInstance *MDM_EASPolicy, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MDM_EASPolicy, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MDM_EASPolicy) SetValues( /* IN */ NamedValuesList string) (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("SetValues" , NamedValuesList);
	if err != nil { return }
