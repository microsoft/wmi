...
type MSFT_MpThreatCatalog struct { 
	*BaseStatus
	...
	func NewMSFT_MpThreatCatalogEx1(instance *cim.WmiInstance) (newInstance *MSFT_MpThreatCatalog, err error) {tmp, err := NewBaseStatusEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_MpThreatCatalog, err error) {tmp, err := NewBaseStatusEx6(hostName, wmiNamespace, userName, password, domainName, query)
