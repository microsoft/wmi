...
type MSFT_MpThreat struct { 
	*BaseStatus
	...
	func NewMSFT_MpThreatEx1(instance *cim.WmiInstance) (newInstance *MSFT_MpThreat, err error) {tmp, err := NewBaseStatusEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_MpThreat, err error) {tmp, err := NewBaseStatusEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_MpThreat) Remove() (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("Remove" );
	if err != nil { return }
