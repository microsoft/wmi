...
type WSP_ReplicationGroup struct { 
	*MSFT_ReplicationGroup
	...
	func NewWSP_ReplicationGroupEx1(instance *cim.WmiInstance) (newInstance *WSP_ReplicationGroup, err error) {tmp, err := NewMSFT_ReplicationGroupEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *WSP_ReplicationGroup, err error) {tmp, err := NewMSFT_ReplicationGroupEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *WSP_ReplicationGroup) CreateReplicationGroup( /* IN */ FriendlyName string,
 ...
 /* OUT */ ExtendedStatus MSFT_StorageExtendedStatus) (result uint32, err error) {retVal, err := instance.InvokeMethod("CreateReplicationGroup" , FriendlyName, Description, StorageElements, LogDevice, LogSizeInBytes, ReplicationQuorum, SyncMode)
	if err != nil { return }
