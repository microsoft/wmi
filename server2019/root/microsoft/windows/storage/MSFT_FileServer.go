...
type MSFT_FileServer struct { 
	*MSFT_StorageObject
	...
	func NewMSFT_FileServerEx1(instance *cim.WmiInstance) (newInstance *MSFT_FileServer, err error) {tmp, err := NewMSFT_StorageObjectEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_FileServer, err error) {tmp, err := NewMSFT_StorageObjectEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_FileServer) CreateFileShare( /* IN */ Name string,
 ...
 /* OUT */ ExtendedStatus MSFT_StorageExtendedStatus) (result uint32, err error) {retVal, err := instance.InvokeMethod("CreateFileShare" , Name, Description, SourceVolume, VolumeRelativePath, ContinuouslyAvailable, EncryptData, FileSharingProtocol, RunAsJob)
	if err != nil { return }
