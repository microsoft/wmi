...
type MSFT_InitiatorPort struct { 
	*cim.WmiInstance
	...
	func NewMSFT_InitiatorPortEx1(instance *cim.WmiInstance) (newInstance *MSFT_InitiatorPort, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_InitiatorPort, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_InitiatorPort) SetNodeAddress( /* IN */ NodeAddress string,
 /* OUT */ ExtendedStatus MSFT_StorageExtendedStatus) (result uint32, err error) {retVal, err := instance.InvokeMethod("SetNodeAddress" , NodeAddress)
	if err != nil { return }
