...
type MSFT_StorageTier struct { 
	*MSFT_StorageObject
	...
	func NewMSFT_StorageTierEx1(instance *cim.WmiInstance) (newInstance *MSFT_StorageTier, err error) {tmp, err := NewMSFT_StorageObjectEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_StorageTier, err error) {tmp, err := NewMSFT_StorageObjectEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_StorageTier) SetAttributes( /* IN */ ProvisioningType uint16,
 ...
 /* OUT */ ExtendedStatus MSFT_StorageExtendedStatus) (result uint32, err error) {retVal, err := instance.InvokeMethod("SetAttributes" , ProvisioningType, AllocationUnitSize, MediaType, FaultDomainAwareness, ColumnIsolation, ResiliencySettingName, Usage, PhysicalDiskRedundancy, NumberOfDataCopies, NumberOfGroups, NumberOfColumns, Interleave)
	if err != nil { return }
	retValue := retVal[0].(int32)
	result = uint32(retValue)
	return
