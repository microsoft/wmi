...
type MSFT_SmbMultichannelConnection struct { 
	*cim.WmiInstance
	...
	func NewMSFT_SmbMultichannelConnectionEx1(instance *cim.WmiInstance) (newInstance *MSFT_SmbMultichannelConnection, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_SmbMultichannelConnection, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_SmbMultichannelConnection) Refresh( /* IN */ ServerName string) (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("Refresh" , ServerName);
	if err != nil { return }
