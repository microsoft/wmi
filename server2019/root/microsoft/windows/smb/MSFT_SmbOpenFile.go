...
type MSFT_SmbOpenFile struct { 
	*cim.WmiInstance
	...
	func NewMSFT_SmbOpenFileEx1(instance *cim.WmiInstance) (newInstance *MSFT_SmbOpenFile, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_SmbOpenFile, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_SmbOpenFile) ForceClose() (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("ForceClose" );
	if err != nil { return }
