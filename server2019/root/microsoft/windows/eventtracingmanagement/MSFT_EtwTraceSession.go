...
type MSFT_EtwTraceSession struct { 
	*CIM_LogicalElement
	...
	func NewMSFT_EtwTraceSessionEx1(instance *cim.WmiInstance) (newInstance *MSFT_EtwTraceSession, err error) {tmp, err := NewCIM_LogicalElementEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_EtwTraceSession, err error) {tmp, err := NewCIM_LogicalElementEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_EtwTraceSession) Send( /* IN */ DestinationFolder string,
 ...
 /* OUT */ ErrorCode uint32) (result uint32, err error) {retVal, err := instance.InvokeMethod("Send" , DestinationFolder, DeleteFile)
	if err != nil { return }
