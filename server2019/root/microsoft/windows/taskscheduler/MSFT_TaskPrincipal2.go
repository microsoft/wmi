...
type MSFT_TaskPrincipal2 struct { 
	*MSFT_TaskPrincipal
	...
	func NewMSFT_TaskPrincipal2Ex1(instance *cim.WmiInstance) (newInstance *MSFT_TaskPrincipal2, err error) {tmp, err := NewMSFT_TaskPrincipalEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_TaskPrincipal2, err error) {tmp, err := NewMSFT_TaskPrincipalEx6(hostName, wmiNamespace, userName, password, domainName, query)
