...
type MSiSCSIInitiator_iSNSServerClass struct { 
	*cim.WmiInstance
	...
	func NewMSiSCSIInitiator_iSNSServerClassEx1(instance *cim.WmiInstance) (newInstance *MSiSCSIInitiator_iSNSServerClass, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSiSCSIInitiator_iSNSServerClass, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSiSCSIInitiator_iSNSServerClass) Refresh() (result uint32, err error) {retVal, err := instance.InvokeMethodWithReturn("Refresh" );
	if err != nil { return }
