...
type StackWalk_Key struct { 
	*StackWalk
	...
	func NewStackWalk_KeyEx1(instance *cim.WmiInstance) (newInstance *StackWalk_Key, err error) {tmp, err := NewStackWalkEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *StackWalk_Key, err error) {tmp, err := NewStackWalkEx6(hostName, wmiNamespace, userName, password, domainName, query)
