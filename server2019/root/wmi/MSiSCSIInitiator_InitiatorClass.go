...
type MSiSCSIInitiator_InitiatorClass struct { 
	*cim.WmiInstance
	...
	func NewMSiSCSIInitiator_InitiatorClassEx1(instance *cim.WmiInstance) (newInstance *MSiSCSIInitiator_InitiatorClass, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSiSCSIInitiator_InitiatorClass, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
