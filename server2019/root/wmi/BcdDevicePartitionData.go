...
type BcdDevicePartitionData struct { 
	*BcdDeviceData
	...
	func NewBcdDevicePartitionDataEx1(instance *cim.WmiInstance) (newInstance *BcdDevicePartitionData, err error) {tmp, err := NewBcdDeviceDataEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *BcdDevicePartitionData, err error) {tmp, err := NewBcdDeviceDataEx6(hostName, wmiNamespace, userName, password, domainName, query)
