...
type StackWalk_Event struct { 
	*StackWalk
	...
	func NewStackWalk_EventEx1(instance *cim.WmiInstance) (newInstance *StackWalk_Event, err error) {tmp, err := NewStackWalkEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *StackWalk_Event, err error) {tmp, err := NewStackWalkEx6(hostName, wmiNamespace, userName, password, domainName, query)
