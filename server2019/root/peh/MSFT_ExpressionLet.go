...
type MSFT_ExpressionLet struct { 
	*MSFT_Expression
	...
	func NewMSFT_ExpressionLetEx1(instance *cim.WmiInstance) (newInstance *MSFT_ExpressionLet, err error) {tmp, err := NewMSFT_ExpressionEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_ExpressionLet, err error) {tmp, err := NewMSFT_ExpressionEx6(hostName, wmiNamespace, userName, password, domainName, query)
