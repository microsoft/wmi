...
type MSFT_Pipeline struct { 
	*cim.WmiInstance
	...
	func NewMSFT_PipelineEx1(instance *cim.WmiInstance) (newInstance *MSFT_Pipeline, err error) {tmp, err := instance, nil
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *MSFT_Pipeline, err error) {tmp, err := instance.GetWmiInstance(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *MSFT_Pipeline) Execute( /* IN */ pipelineExecutionName string,
 /* OUT */ pipelineExecution MSFT_PipelineExecution) (result uint32, err error) {retVal, err := instance.InvokeMethod("Execute" , pipelineExecutionName)
	if err != nil { return }
