...
type TraceFailedRequestsSection struct { 
	*ConfigurationSectionWithCollection
	...
	func NewTraceFailedRequestsSectionEx1(instance *cim.WmiInstance) (newInstance *TraceFailedRequestsSection, err error) {tmp, err := NewConfigurationSectionWithCollectionEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *TraceFailedRequestsSection, err error) {tmp, err := NewConfigurationSectionWithCollectionEx6(hostName, wmiNamespace, userName, password, domainName, query)
