...
type ManyToOneSettings struct { 
	*EmbeddedObject
	...
	func NewManyToOneSettingsEx1(instance *cim.WmiInstance) (newInstance *ManyToOneSettings, err error) {tmp, err := NewEmbeddedObjectEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *ManyToOneSettings, err error) {tmp, err := NewEmbeddedObjectEx6(hostName, wmiNamespace, userName, password, domainName, query)
