...
type SSLBinding struct { 
	*Object
	...
	func NewSSLBindingEx1(instance *cim.WmiInstance) (newInstance *SSLBinding, err error) {tmp, err := NewObjectEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *SSLBinding, err error) {tmp, err := NewObjectEx6(hostName, wmiNamespace, userName, password, domainName, query)
...
func (instance *SSLBinding) Create( /* IN */ IPAddress string,
 ...
 /* IN */ StoreName string) (err error) {_, err = instance.InvokeMethodWithReturn("Create" , IPAddress, Port, CertificateHash, StoreName);
	if err != nil { return }
	return
