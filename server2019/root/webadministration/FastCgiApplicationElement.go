...
type FastCgiApplicationElement struct { 
	*CollectionElement
	...
	func NewFastCgiApplicationElementEx1(instance *cim.WmiInstance) (newInstance *FastCgiApplicationElement, err error) {tmp, err := NewCollectionElementEx1(instance)
	...
	password string,
	domainName string,
	query *query.WmiQuery ) (newInstance *FastCgiApplicationElement, err error) {tmp, err := NewCollectionElementEx6(hostName, wmiNamespace, userName, password, domainName, query)
