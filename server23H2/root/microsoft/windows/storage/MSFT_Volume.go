...
func (instance *MSFT_Volume) Format( /* IN */ FileSystem string,
	...
	/* OUT */ CreatedStorageJob MSFT_StorageJob,
	/* OUT */ ExtendedStatus MSFT_StorageExtendedStatus) (result uint32, err error) {
	retVal, err := instance.InvokeMethod("Format", FileSystem, FileSystemLabel, AllocationUnitSize, Full, Force, Compress, ShortFileNameSupport, SetIntegrityStreams, UseLargeFRS, DisableHeatGathering, IsDAX, NoTrim, DevDrive, SHA256Checksums, RunAsJob)
	if err != nil {
		return
	}
	retValue := retVal[0].(int32)
...
func (instance *MSFT_Volume) Repair( /* IN */ OfflineScanAndFix bool,
	...
	/* OUT */ CreatedStorageJob MSFT_StorageJob,
	/* OUT */ ExtendedStatus MSFT_StorageExtendedStatus) (result uint32, err error) {
	retVal, err := instance.InvokeMethod("Repair", OfflineScanAndFix, Scan, SpotFix, DetectLeaks, ScratchFile, Threads, Triage, DirectoryIds, Salvage, ScratchDir, TargetFile, TargetDir, RunAsJob)
	if err != nil {
		return
	}
	retValue := retVal[0].(int32)
