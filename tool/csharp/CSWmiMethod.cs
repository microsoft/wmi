...
{
    public class CSWmiMethod : WmiMethod
    {
        public CSWmiMethod() { }
        /// <summary>
        /// 
        ...
        public CSWmiMethod(MethodData mData, CSWmiClass wClass)
            : base(mData, wClass)
        {
            HeaderComments.Add(CSWmiSource.GetHeaderCommentText(mData.Qualifiers));
            ...
            if (collection != null)
            {
                ...
                foreach (var pData in collection)
                {
                    sbComment.AppendFormat(CultureInfo.InvariantCulture,
                            ...
                            CSharpFormat.GetDescription(pData.Qualifiers).Replace("\n", "\n///"),
                            pData.IsArray ? "[]" : "");
                }
            }
        ...
        {
            return new CSWmiMethodParam(pData, pType, wMethod as CSWmiMethod, optional);
        }

        protected override WmiMethodParam GetWmiMethodParam(string comment, ParamType pType, string Type, string Name, bool custom)
        {
            return new CSWmiMethodParam(comment, pType, Type, Name, custom);
        }
        ...
        protected override string GetMethodBodyText(MethodData mData)
        {
            ...
            if (CSharpFormat.HasOutParams(mData))
            {
                ...
                foreach (var pData in mData.OutParameters.Properties)
                {
                    if (!pData.Name.Equals("ReturnValue", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (CSharpFormat.HasJobOutputParams(mData) && tmpType.IsClass
